...
Highcharts.chart('container', {
    ...
    title: {
        text: 'Reporte Precios de Calzado',
        align: 'left'
    },
    /*subtitle: {
        text: 'Source: ' +
    ...
    series: [{
        type: 'pie',
        /*name: 'Share',*/
        name: 'Porcentaje de Precios de Calzados',
        data: [
            /*['Samsung', 23],
            ['Apple', 18],
            {
            ...
            ['Others', 30]*/
            <?php
            $sql =$db->Prepare("SELECT tipo_calzado, precio FROM modelos WHERE estado ='A'");
            $rs = $db->GetAll($sql);
            foreach($rs as $k => $fila)
            {
                ?>
                ['<?php echo $fila["tipo_calzado"]; ?>', <?php echo $fila["precio"]; ?>],
                <?php
            }    
            ?>
